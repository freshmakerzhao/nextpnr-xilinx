...
struct FasmBackend
{
    ...
    FasmBackend(Context *ctx, std::vector<Tool::byte_t>* buffer,std::ostream *out = nullptr) : ctx(ctx), buffer(buffer){
        if (!buffer) {
    ...
    FasmBackend(Context *ctx, std::ostream *out) : ctx(ctx), out(out){
        if (!out) {
            throw std::invalid_argument("Output stream pointer cannot be null");
        }
    };
    ...
    std::vector<IdString> get_inputs(CellInfo *cell)
    {
        ...
        if (type == ctx->id("LUT1"))
            ...
            return {ctx->id("I0"), ctx->id("I1"), ctx->id("I2"), ctx->id("I3"), ctx->id("I4"), ctx->id("I5")};
        else if (type == ctx->id("ROM32X1"))
            return {ctx->id("A0"), ctx->id("A1"), ctx->id("A2"), ctx->id("A3"), ctx->id("A4")};
        else if (type == ctx->id("ROM64X1"))
            return {ctx->id("A0"), ctx->id("A1"), ctx->id("A2"), ctx->id("A3"), ctx->id("A4"), ctx->id("A5")};
        else if (type == ctx->id("ROM128X1"))
            return {ctx->id("A0"), ctx->id("A1"), ctx->id("A2"), ctx->id("A3"), ctx->id("A4"), ctx->id("A5"), ctx->id("A6")};
        else if (type == ctx->id("ROM256X1"))
            return {ctx->id("A0"), ctx->id("A1"), ctx->id("A2"), ctx->id("A3"), ctx->id("A4"), ctx->id("A5"), ctx->id("A6"), ctx->id("A7")};
        else if (type == ctx->id("RAMD64E"))
            return {ctx->id("RADR0"), ctx->id("RADR1"), ctx->id("RADR2"),
                    ctx->id("RADR3"), ctx->id("RADR4"), ctx->id("RADR5")};
        else if (type == ctx->id("RAMS64E"))//[lgl] get inputs of RAMS64E                                
            return {ctx->id("ADR0"), ctx->id("ADR1"), ctx->id("ADR2"),                                   
                    ctx->id("ADR3"), ctx->id("ADR4"), ctx->id("ADR5")};    
        ...
        else if (type == ctx->id("RAMS32"))//[lgl] get inputs of RAMS32                                  
            return {ctx->id("ADR0"), ctx->id("ADR1"), ctx->id("ADR2"), ctx->id("ADR3"), ctx->id("ADR4")};            
        else
    ...
    void write_clocking()
    {
        ...
        for (auto cell : sorted(ctx->cells)) {
            ...
            if (ci->type == id_BUFGCTRL) {
                ...
                write_bit("IN_USE");
                std::string origin_type = str_or_default(ci->attrs, ctx->id("X_ORIG_TYPE"), "");
                if(origin_type == "BUFGMUX_1" ||origin_type == "BUFGCE_1")
                {
                ...
                write_bit("IN_USE");
                push("BUFR_DIVIDE");
                if (ci->params[ctx->id("BUFR_DIVIDE")].str == "BYPASS")
                    write_bit("BYPASS");
                else
        ...
        for (int tile = 0; tile < int(tt.size()); tile++) {
            ...
            if (type == "HCLK_L" || type == "HCLK_R" || type == "HCLK_L_BOT_UTURN" || type == "HCLK_R_BOT_UTURN") {
                ...
                for (auto s : used_sources) {
                    ...
                    if (boost::contains(s, "BUFRCLK")) {
                        write_bit(s);
                        hclk_by_row[tile / ctx->chip_info->width].insert(s.substr(s.find("BUFRCLK")));
                    }
                }
                pop();
