...
void XC7Packer::walk_dsp(CellInfo *root, CellInfo *current_cell, int constr_z) 
{
    ...
    static int dsp_count = 1; // 静态变量用于跟踪放置的DSP数量
    dsp_count++; // 增加DSP数量计数器
    ...
    if (cascaded_cell != nullptr) {
        ...
        if(dsp_count %2 != 0 && dsp_count != 2)
            previous_y = current_cell->constr_y - 5;
        if((dsp_count-1) %10 == 0)
            previous_y -= 1;
        cascaded_cell->constr_y = previous_y;
        cascaded_cell->constr_z = dsp_count%2 == 0 ? BEL_UPPER_DSP : BEL_LOWER_DSP;
