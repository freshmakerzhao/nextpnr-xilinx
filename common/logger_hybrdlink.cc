...
std::string LogCategoryToString(LogCategory category) {
    switch (category) {
        ...
        case LogCategory::CORETCL:
            return "Coretcl";
        case LogCategory::HYBRDLINK_TCL:
            return "Hybrdlink_tcl";
        case LogCategory::COMMON:
            return "Common";
        case LogCategory::CONSTRAINTS:
            return "Constraints";
        ...
        case LogCategory::NETLIST:
            return "Netlist";
        case LogCategory::OPT:
            return "Opt";
        case LogCategory::TIMING:
            return "Timing";
        case LogCategory::PACK:
            return "pack";
...
std::string PhaseTypeToString(PhaseType phaseType) {
	switch (phaseType) {
        ...
        case PhaseType::ROUTE:
			return "route";
		default:
			throw std::invalid_argument("Invalid PhaseType value");
	}
}

// 创建Log结构体
LogData LogData::CreateLogStruct(const LevelCode& levelCode,const LogCategory& categoryInfo,const PhaseType& phaseType,
        const std::string& taskInfo) {
    std::string category = "[" + LogCategoryToString(categoryInfo) + " " + std::to_string(static_cast<int>(categoryInfo)) + "-" + std::to_string(Common::GetNextIndex(LogCategoryToString(categoryInfo))) + "]";
    LogData logData;
    ...
    logData.sub_phase = PhaseTypeToString(phaseType);
